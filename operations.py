...
def Remove_item_from_wishlist(Username,product_ID,gamers_json_file):
    ...
    for i in range(len(data)):
        if data[i]["Username"]==Username:
            data[i]["Wishlist"].remove(product_ID)
            f.seek(0)
            f.truncate()
            json.dump(data,f)
    f.close()
    ...
    for i in range(len(data)):
        if data[i]["Username"]==Username:
            for j in range(len(data[i]["Cart"])):
                if data[i]["Cart"][j]["Product ID"]==Order_Id:
                    data[i]["Cart"][j]["Quantity"]-=1
                    if data[i]["Cart"][j]["Quantity"]==0:
                        data[i]["Cart"].remove(data[i]["Cart"][j])
                    break
            f.seek(0)
            f.truncate()
            json.dump(data,f)
